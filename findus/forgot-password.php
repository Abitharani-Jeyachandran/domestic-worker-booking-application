...
$hashednewpass=password_hash($password, PASSWORD_BCRYPT, $options);
    ...
    $sql ="SELECT id FROM tbljobseekers WHERE EmailId=:email";
...
if($query->rowCount() > 0)
{
$sql="update tbljobseekers set Password=:hashednewpass WHERE EmailId=:email";
$query = $dbh->prepare($sql);
// Binding Post Values
...
<head>
<title>JobSeeker Reset Your Account Password | FIND US</title>
